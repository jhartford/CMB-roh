...
echo "Beginning runs of homozygosity script"
if [ "$3" = "" ]
then
	if [ "$2" = "no-prune" ]
	then
...
if [ "$3" = "nobin" ]
then
	echo "Running with human readable file"
	if [ "$2" = "no-prune" ]
	then
		plink --noweb --file "$1" --out roh --homozyg --homozyg-snp 45 --homozyg-window-het 1 --homozyg-window-missing 5 --homozyg-window-threshold 0.05
	else
		echo "Pruning..."
	    if [ ! -f "$1-pruned.prune.out" ]
		then
			plink --noweb --file "$1" --out "$1-pruned" --indep 50 5 2
		fi
		plink --noweb --file "$1" --out "$1-roh" --exclude "$1-pruned.prune.out" --homozyg --homozyg-snp 45 --homozyg-window-het 1 --homozyg-window-missing 5 --homozyg-window-threshold 0.05  	
	fi
fi